const users = [
    ...
    { id: 'e789565f-fa5a-4d5e-8f6c-dd126cf995be', firstName: 'Madison', lastName: 'Lambert' ,text:'Great Job!'}
];
...
let a= users.filter(result=>result.firstName==='Madison' && result.lastName==='Marshall')
console.log(a)
//or
const b =users.find(result=>result.firstName==='Madison' && result.lastName==='Marshall')
console.log(b)


//// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
const q=users.find(output=> output.id === comments[0].userId)
console.log(q)

// 3) Which user commented 'OK great thanks'?
const r = comments.find(output=> output.text==='OK great thanks').userId
const s =users.find(user=> user.id=== r)
console.log(s)


// 4) Add the user's first and last name to each comment in the comments array
const t=comments.map(test=>{
    const {firstName,lastName} =users.find(result=> result.id === test.userId)
